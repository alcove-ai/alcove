...
func (jp *JiraPoller) PollAll(ctx context.Context) {
	...
	for rows.Next() {
		...
		// Warn if retrigger_on_comment is enabled without users filter
		if trigger.Jira.RetriggerOnComment && len(trigger.Jira.Users) == 0 {
			log.Printf("jira-poller: WARNING: schedule %s has retrigger_on_comment without users filter — all comments will trigger re-dispatch", name)
		}
	...
	for _, issue := range searchResult.Issues {
		...
		for _, target := range targets {
			if target.trigger.Matches(issueProject, issueComponents, issue.Fields.Labels) {
				...
				// Construct trigger_ref — may include comment count for retrigger_on_comment workflows
				triggerRef := issue.Key
				if target.trigger.RetriggerOnComment {
					// Check if we should use comment-aware trigger_ref
					useCommentAwareTriggerRef := true

					// If users filter is specified, check latest comment author
					if len(target.trigger.Users) > 0 {
						// We need to check the latest comment author against the allowlist
						// This requires enrichment, but we'll do a focused call just for this
						if latestCommentAuthor := jp.getLatestCommentAuthor(ctx, token, issue.Key); latestCommentAuthor != "" {
							// Check if the latest commenter is in the allowlist
							authorMatches := false
							for _, allowedUser := range target.trigger.Users {
								if strings.EqualFold(allowedUser, latestCommentAuthor) {
									authorMatches = true
									break
								}
							}
							if !authorMatches {
								// Latest commenter not in allowlist, fall back to plain issue key
								useCommentAwareTriggerRef = false
							}
						} else {
							// No comments found, use plain issue key
							useCommentAwareTriggerRef = false
						}
					}
				...
				// Exclude failed/error/cancelled runs to allow retries after failures.
				var count int
				jp.db.QueryRow(ctx, `
				...
				if count > 0 {
					log.Printf("jira-poller: skipping %s - already dispatched recently (blocking run count: %d)", triggerRef, count)
...
// getLatestCommentAuthor fetches the latest comment for an issue and returns the author's display name.
// Returns empty string if no comments are found or if there's an error.
func (jp *JiraPoller) getLatestCommentAuthor(ctx context.Context, token, issueKey string) string {
	...
	if err != nil {
		log.Printf("jira-poller: error fetching latest comment for %s: %v", issueKey, err)
		return ""
	}
	...
	if err := json.Unmarshal(data, &commentsResult); err != nil {
		log.Printf("jira-poller: error parsing latest comment for %s: %v", issueKey, err)
		return ""
	}

	if len(commentsResult.Comments) == 0 {
		return ""
	}

	// Return display name, fall back to email address if display name is empty
	author := commentsResult.Comments[0].Author
